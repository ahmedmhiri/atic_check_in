...
export default function ScanResult({
  ...
  if (!result) {
    return (
      <div className="flex min-h-[4.5rem] items-center rounded-lg border border-dashed border-slate-300 bg-white px-4 text-sm text-slate-500">
        {idleText}
      </div>
    );
  }
  ...
  return (
    <div
      ...
      role="status"
      aria-live="assertive"
      className={`min-h-[4.5rem] rounded-lg border px-4 py-3 shadow-sm ${TONES[tone]}`}
    >
